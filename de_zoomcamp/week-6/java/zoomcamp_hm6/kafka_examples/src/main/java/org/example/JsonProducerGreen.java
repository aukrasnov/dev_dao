...
public class JsonProducerGreen {
    ...
    public Stream<GreenTrip> getRides() throws IOException, CsvException {
        var ridesStream = this.getClass().getResource("/green_tripdata_2019-01.csv");
        var reader = new CSVReader(new FileReader(ridesStream.getFile()));
        reader.skip(1);
        return reader.readAll().stream().map(GreenTrip::new);

    }

    public void publishRides(Stream<GreenTrip> rides) throws ExecutionException, InterruptedException {
...
//            ride.tpep_dropoff_datetime = LocalDateTime.now();
            var record = kafkaProducer.send(new ProducerRecord<>(Topics.INPUT_GREEN_TOPIC, String.valueOf(ride.DOLocationID), data), (metadata, exception) -> {
                if(exception != null) {
            ...
            System.out.println("green:" + ride.DOLocationID);
//            Thread.sleep(500);
        });
    }
